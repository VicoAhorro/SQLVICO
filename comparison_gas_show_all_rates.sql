WITH -- ====================== BASE GAS ======================
  calculated_prices_gas AS (
    ...
    ON u.user_id = cg.advisor_id
      LEFT JOIN comparison_rates cr ON cr.type = 'gas'
      ...
      AND (
        ...
        OR (
          cr.invoice_month = cg.invoice_month
      ...
      AND (
        ...
        OR NOT EXISTS (
          -- Fallback: si NO hay ninguna con permanencia,
          ...
          WHERE crp.type = 'gas'
            ...
            AND (
              ...
              OR (
                crp.invoice_month = cg.invoice_month
  ...
  WHERE cg.id = 'cd8b3e68-1396-4751-934c-b84c40ca80ea'
  ),
  unified_calculated_prices AS (
    SELECT *
    FROM calculated_prices_gas
  ),
  -- ====================== CRS Y FÓRMULAS ======================
  unified_extended_prices AS (
    ...
    FROM unified_calculated_prices ucp
      ...
      CROSS JOIN LATERAL (
        SELECT COALESCE(ucp.anual_consumption_p1, 0::real) * COALESCE(ucp."precio_kwh_P1", 0::real) + COALESCE(ucp.anual_consumption_p2, 0::real) * COALESCE(ucp."precio_kwh_P2", 0::real) + COALESCE(ucp.anual_consumption_p3, 0::real) * COALESCE(ucp."precio_kwh_P3", 0::real) + COALESCE(ucp.anual_consumption_p4, 0::real) * COALESCE(ucp."precio_kwh_P4", 0::real) + COALESCE(ucp.anual_consumption_p5, 0::real) * COALESCE(ucp."precio_kwh_P5", 0::real) + COALESCE(ucp.anual_consumption_p6, 0::real) * COALESCE(ucp."precio_kwh_P6", 0::real) AS annual_new_energy_pre_vat
      ) a1
      CROSS JOIN LATERAL (
        SELECT COALESCE(ucp.anual_consumption_p1, 0::real) * COALESCE(ucp.price_cp1, 0::real) + COALESCE(ucp.anual_consumption_p2, 0::real) * COALESCE(ucp.price_cp2, 0::real) + COALESCE(ucp.anual_consumption_p3, 0::real) * COALESCE(ucp.price_cp3, 0::real) + COALESCE(ucp.anual_consumption_p4, 0::real) * COALESCE(ucp.price_cp4, 0::real) + COALESCE(ucp.anual_consumption_p5, 0::real) * COALESCE(ucp.price_cp5, 0::real) + COALESCE(ucp.anual_consumption_p6, 0::real) * COALESCE(ucp.price_cp6, 0::real) AS annual_old_energy_pre_vat
      ) a2
      CROSS JOIN LATERAL (
        SELECT (
      ...
      CROSS JOIN LATERAL (
        SELECT (
            a1.annual_new_energy_pre_vat + pwr_year.annual_new_power_pre_vat
          ) AS annual_new_pre_vat,
          (
            a2.annual_old_energy_pre_vat + pwr_year.annual_old_power_pre_vat
          ) AS annual_old_pre_vat
      ) base
      CROSS JOIN LATERAL (
        SELECT base.annual_new_pre_vat * (
      ...
      CROSS JOIN LATERAL (
        SELECT CASE
            WHEN ucp.tarifa_plana = TRUE THEN COALESCE(ucp.current_total_invoice, 0::real) * (365.0 / NULLIF(ucp.days::numeric, 0))::double precision - (
              COALESCE(ucp.anual_consumption_p1, 0::real) * (COALESCE(ucp.price_cp1, 0::real) + 0.00234) + COALESCE(
                NULLIF(ucp.power_p1, 0::double precision),
                1::real
              ) * COALESCE(ucp.price_pp1, 0::real) * 365::double precision + COALESCE(
                NULLIF(ucp.power_p2, 0::double precision),
                1::real
  ...
  ranked_comparisons AS (
    SELECT uep.*,
      CASE
        ...
        AND uep.savings_yearly > 0 THEN uep.savings_yearly + COALESCE(uep.total_crs, 0) * 4::double precision
...
SELECT -- (1) Basales + consumos
  ...
  (
    ...
    )::double precision + COALESCE(rc.equipment_rental, 0::double precision)
  ) * (
  ...
  CASE
    ...
    and rc.type <> '3_0'::text then (
      (
        COALESCE(rc.anual_consumption_p1, 0::real) * COALESCE(rc."precio_kwh_P1", 0::real) + COALESCE(rc.anual_consumption_p2, 0::real) * COALESCE(rc."precio_kwh_P2", 0::real) + COALESCE(rc.anual_consumption_p3, 0::real) * COALESCE(rc."precio_kwh_P3", 0::real) + COALESCE(rc.anual_consumption_p4, 0::real) * COALESCE(rc."precio_kwh_P4", 0::real) + COALESCE(rc.anual_consumption_p5, 0::real) * COALESCE(rc."precio_kwh_P5", 0::real) + COALESCE(rc.anual_consumption_p6, 0::real) * COALESCE(rc."precio_kwh_P6", 0::real) + COALESCE(
          NULLIF(rc.power_p1, 0::double precision),
          1::real
        ) * COALESCE(rc."precio_kw_P1", 0::real) * 365::double precision + COALESCE(rc.power_p2, 0::real) * COALESCE(rc."precio_kw_P2", 0::real) * 365::double precision + COALESCE(rc.power_p3, 0::real) * COALESCE(rc."precio_kw_P3", 0::real) * 365::double precision + COALESCE(rc.power_p4, 0::real) * COALESCE(rc."precio_kw_P4", 0::real) * 365::double precision + COALESCE(rc.power_p5, 0::real) * COALESCE(rc."precio_kw_P5", 0::real) * 365::double precision + COALESCE(rc.power_p6, 0::real) * COALESCE(rc."precio_kw_P6", 0::real) * 365::double precision - COALESCE(rc.surpluses, 0::real) * 182.5::double precision / case
        ...
        COALESCE(rc.anual_consumption_p1, 0::real) * COALESCE(rc.price_cp1, 0::real) + COALESCE(rc.anual_consumption_p2, 0::real) * COALESCE(rc.price_cp2, 0::real) + COALESCE(rc.anual_consumption_p3, 0::real) * COALESCE(rc.price_cp3, 0::real) + COALESCE(rc.anual_consumption_p4, 0::real) * COALESCE(rc.price_cp4, 0::real) + COALESCE(rc.anual_consumption_p5, 0::real) * COALESCE(rc.price_cp5, 0::real) + COALESCE(rc.anual_consumption_p6, 0::real) * COALESCE(rc.price_cp6, 0::real) + COALESCE(
          NULLIF(rc.power_p1, 0::double precision),
          1::real
        ) * COALESCE(rc.price_pp1, 0::real) * 365::double precision + COALESCE(rc.power_p2, 0::real) * COALESCE(rc.price_pp2, 0::real) * 365::double precision + COALESCE(rc.power_p3, 0::real) * COALESCE(rc.price_pp3, 0::real) * 365::double precision + COALESCE(rc.power_p4, 0::real) * COALESCE(rc.price_pp4, 0::real) * 365::double precision + COALESCE(rc.power_p5, 0::real) * COALESCE(rc.price_pp5, 0::real) * 365::double precision + COALESCE(rc.power_p6, 0::real) * COALESCE(rc.price_pp6, 0::real) * 365::double precision - COALESCE(rc.surpluses, 0::real) * 182.5::double precision / case
          when COALESCE(rc.days::real, 0::real) = 0::double precision then 182.5
          else rc.days::numeric
  ...
  LOWER(
    COALESCE(rc."CUPS", '') || ' ' || COALESCE(rc.client_email, '') || ' ' || COALESCE(rc.company, '') || ' ' || COALESCE(rc.rate_name, '') || ' ' || COALESCE(rc.temp_client_name, '') || ' ' || COALESCE(rc.temp_client_last_name, '')
  ) AS search,
