create view public._comparisons_detailed_light as
with
  base as (
    select
      cl.id,
    ...
    from
      (
        select
          ...
          comparison_light.client_email,
          comparison_light.advisor_id,
          comparison_light.consumption_p1,
          comparison_light.consumption_p2,
          comparison_light.consumption_p3,
          comparison_light.power_p1,
          comparison_light.power_p2,
          comparison_light.current_total_invoice,
          comparison_light.surpluses,
          comparison_light."VAT",
          comparison_light.power_days,
          comparison_light.pdf_invoice,
          comparison_light.proposal_date,
          comparison_light."CUPS",
          comparison_light.address_id,
          comparison_light.company,
          comparison_light.rate_name,
          comparison_light.invoice_month,
          comparison_light.equipment_rental,
          comparison_light.selfconsumption,
          comparison_light.manual_data,
          comparison_light.valuation_id,
          comparison_light.invoice_year,
          comparison_light.temp_client_name,
          comparison_light.temp_client_last_name,
          comparison_light.deleted,
          comparison_light.deleted_reason,
          comparison_light.preferred_subrate,
          comparison_light.anual_consumption_p1,
          comparison_light.anual_consumption_p2,
          comparison_light.anual_consumption_p3,
          comparison_light.max_power,
          comparison_light."precio_kwh_P1",
          comparison_light."precio_kwh_P2",
          comparison_light."precio_kwh_P3",
          comparison_light."precio_kw_P1",
          comparison_light."precio_kw_P2",
          comparison_light.autoconsumo_precio,
          comparison_light.totalconsumo,
          comparison_light.totalpotencia,
          comparison_light.tarifa_plana,
          comparison_light.cif,
          comparison_light.region,
          comparison_light.source_type_id,
          comparison_light.wants_permanence,
          comparison_light.total_excedentes_precio,
          comparison_light.comparison_id,
          comparison_light.invoice_address,
          comparison_light.term_month_i_want,
          comparison_light.deleted_at,
          comparison_light.excluded_company_ids,
          comparison_light.wants_gdo,
          comparison_light.temp_client_phone
        from
          comparison_light
        where
          comparison_light.valuation_id is null
          and comparison_light.deleted = false
      ) cl
    ...
    where
      (
        cr.rate_mode <> 'Indexada'::rate_mode_type
        and (
          cl.preferred_subrate is null
          or cl.preferred_subrate = ''::text
          or lower(cr.subrate_name) = lower(cl.preferred_subrate)
        )
        or cr.rate_mode = 'Indexada'::rate_mode_type
        and cr.invoice_month = cl.invoice_month
        ...
        or not (
          cr.company in (
            ...
            from
              companies c_ex
            where
              c_ex.id = any (cl.excluded_company_ids)
          )
        )
      )
      and (
      ...
      and (
        cl.wants_permanence is null
        or cr.has_permanence = cl.wants_permanence
      )
      and (
        cl.wants_permanence is not true
        or cl.term_month_i_want is null
        or cr.term_month <= cl.term_month_i_want
      )
      and (
        cl.wants_gdo = false
        or cr.has_gdo = true
  ...
  with_crs as (
    ...
    from
      tot t
      left join comparison_rates_crs crs on crs.comparison_rate_id = t.new_rate_id
      and (
  ...
  ranked as (
    select
      ...
      rp.excluded_company_ids,
      rp.wants_gdo,
      rp.temp_client_phone,
      rp.new_rate_id,
      rp.new_company,
      rp.new_rate_name,
      rp.new_subrate_name,
...
select distinct
  ...
  array[COALESCE(rc.advisor_email, ''::text), 'All'::text] as advisor_filter,
  EXTRACT(
    month
    from
      rc.created_at
  ...
  (
    (
      (
        (
          (COALESCE(rc."CUPS", ''::text) || ' '::text) || COALESCE(rc.advisor_display_name, ''::text)
        ) || ' '::text
      ) || COALESCE(rc.advisor_email, ''::text)
    ) || ' '::text
  ) || lower(
    (
      (
