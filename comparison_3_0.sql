...
calculated_prices_3_0 AS (
  SELECT
    ...
    COALESCE(c30.consumption_p1, 0.0::real) + COALESCE(c30.consumption_p2, 0.0::real) + COALESCE(c30.consumption_p3, 0.0::real) + COALESCE(c30.consumption_p4, 0.0::real) + COALESCE(c30.consumption_p5, 0.0::real) + COALESCE(c30.consumption_p6, 0.0::real) AS total_consumption,
    COALESCE(c30.anual_consumption_p1, 0.0::real) + COALESCE(c30.anual_consumption_p2, 0.0::real) + COALESCE(c30.anual_consumption_p3, 0.0::real) + COALESCE(c30.anual_consumption_p4, 0.0::real) + COALESCE(c30.anual_consumption_p5, 0.0::real) + COALESCE(c30.anual_consumption_p6, 0.0::real) AS total_anual_consumption,
    ...
    -- total_power_price (término de potencia/días por periodos P1..P6)
    ...
    -- total_consumption_price (término de energía por periodos C1..C6)
    ...
    -- new_total_price (suma de potencia + energía +/- excedentes)
    COALESCE(c30.power_p1, 0::real) * COALESCE(cr.price_pp1, 0::real) * COALESCE(c30.power_days, 0)::double precision +
    COALESCE(c30.power_p2, 0::real) * COALESCE(cr.price_pp2, 0::real) * COALESCE(c30.power_days, 0)::double precision +
    COALESCE(c30.power_p3, 0::real) * COALESCE(cr.price_pp3, 0::real) * COALESCE(c30.power_days, 0)::double precision +
    COALESCE(c30.power_p4, 0::real) * COALESCE(cr.price_pp4, 0::real) * COALESCE(c30.power_days, 0)::double precision +
    ...
    COALESCE(c30.consumption_p5, 0::real) * COALESCE(cr.price_cp5, 0::real) +
    COALESCE(c30.consumption_p6, 0::real) * COALESCE(cr.price_cp6, 0::real) +
    COALESCE(c30.power_surpluses, 0::real) -
...
unified_extended_prices AS (
  SELECT
      ...
      WHEN ucp.new_company IS NOT NULL THEN
        (
          (COALESCE(ucp.anual_consumption_p1, 0::real) * COALESCE(ucp."precio_kwh_P1", 0::real) +
           ...
           - COALESCE(ucp.surpluses, 0::real) * 182.5::double precision / NULLIF(ucp.days::numeric, 0::numeric)::double precision * COALESCE(ucp.autoconsumo_precio, 0::real)
          ) * 1.05113::double precision * (1::double precision + COALESCE(ucp."VAT", 0::real))
          -
          (
            COALESCE(ucp.total_consumption_price, 0::real) / NULLIF(ucp.total_consumption, 0::real) * COALESCE(ucp.total_anual_consumption, 0::real)
            + COALESCE(ucp.power_p1, 0::real) * COALESCE(ucp.price_pp1, 0::real) * 365::double precision
    ...
    -- savings mensual (mantenemos naming como en tu vista grande)
    CASE
      WHEN ucp.new_company IS NOT NULL THEN
...
all_comparisons_ranked AS (
  -- Solo 3_0 (no unimos phone/light/gas)
...
SELECT DISTINCT
  ...
  -- Precio nuevo mensual con IVA (solo 3_0)
  (
  ...
  )::numeric::double precision AS new_total_price_with_vat,

  -- Precio nuevo anual con IVA (solo 3_0)
  (
    (
      COALESCE(rc.anual_consumption_p1, 0::real) * COALESCE(rc.price_cp1, 0::real) +
      COALESCE(rc.anual_consumption_p2, 0::real) * COALESCE(rc.price_cp2, 0::real) +
  ...
  -- Porcentaje de ahorro (solo rama 3_0 de tu cálculo)
  (
    (
      (
  ...
  -- Supervisores (se mantiene la misma lógica que tu vista original)
  CASE
    WHEN (
      SELECT u2.racc FROM users u2 WHERE u2.user_id = rc.advisor_id LIMIT 1
    ) = TRUE THEN (
  ...
  TO_CHAR(rc.created_at, 'MM')  AS created_month,
  TO_CHAR(rc.created_at, 'YYYY') AS created_year,
  concat_ws(', ', rc.client_email, rc."CUPS") AS search,
  ARRAY[rc.company, 'All'::text] AS company_filter

FROM
  all_comparisons_ranked rc
  LEFT JOIN _users_supervisors us
    ON rc.advisor_id = us.user_id
WHERE
  rc.rank = 1
