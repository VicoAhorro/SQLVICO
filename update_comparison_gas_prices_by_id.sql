...
BEGIN
  SELECT
    ...
    COALESCE(c."VAT", 0),
    COALESCE(c.iee, 0.00234),
    COALESCE(c.equipment_rental, 0)
  INTO
    v_totalfijo, v_totalconsumo, v_days, v_consumption, v_vat, v_iee, v_equipment
  FROM public.comparison_gas c
  WHERE c.id = _id AND NOT c.deleted;

  -- Impuesto Especial sobre Hidrocarburos: €/kWh leído de comparison_gas.iee
  -- (default 0.00234 €/kWh).
  v_impuesto_gas := v_consumption * v_iee;
