DECLARE
  ...
  v_cons2 NUMERIC := 0;
  v_cons3 NUMERIC := 0;
  v_total_excedentes_precio NUMERIC := 0; -- ✅ Renombrada
...
BEGIN
  SELECT
    ...
    COALESCE(c.total_excedentes_precio, 0), -- ✅ Origen correcto
  ...
  INTO
    ...
    v_total_excedentes_precio, v_surpluses
  ...
  v_total_final := ROUND((((v_totalconsumo + v_totalpotencia - v_total_excedentes_precio) * (1 + v_iee)) + v_equipment) * (1 + v_vat), 2);
  ...
  SET
    ...
    "autoconsumo_precio" = CASE
                            WHEN v_surpluses <> 0
                            THEN v_total_excedentes_precio / v_surpluses
                            ELSE 0
                          END,
