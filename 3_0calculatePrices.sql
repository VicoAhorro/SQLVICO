DECLARE
  ...
  precio_kwh_tbl_1 NUMERIC := 0;
  precio_kwh_tbl_2 NUMERIC := 0;
  precio_kwh_tbl_3 NUMERIC := 0;
  precio_kwh_tbl_4 NUMERIC := 0;
  precio_kwh_tbl_5 NUMERIC := 0;
  precio_kwh_tbl_6 NUMERIC := 0;
...
BEGIN
  ...
  -- Si el cliente quiere fija, recalculamos el total variable con los precios kWh guardados
  IF rate_i_have_var = 'Fija' THEN
    totalvariable :=
      COALESCE(consumo_1, 0) * COALESCE(precio_kwh_tbl_1, 0) +
      COALESCE(consumo_2, 0) * COALESCE(precio_kwh_tbl_2, 0) +
      COALESCE(consumo_3, 0) * COALESCE(precio_kwh_tbl_3, 0) +
      COALESCE(consumo_4, 0) * COALESCE(precio_kwh_tbl_4, 0) +
      COALESCE(consumo_5, 0) * COALESCE(precio_kwh_tbl_5, 0) +
      COALESCE(consumo_6, 0) * COALESCE(precio_kwh_tbl_6, 0);
  END IF;
